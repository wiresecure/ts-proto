...
function generateService(typeMap, fileDesc, sourceInfo, serviceDesc, options) {
    ...
    serviceDesc.method.forEach((methodDesc, index) => {
        ...
        if (methodDesc.serverStreaming) {
            requestFn = requestFn.addParameter('onMessage', new ts_poet_1.Lambda(new Map([['msg', outputType]]), ts_poet_1.TypeNames.VOID));
            requestFn = requestFn.addParameter('onEnd', new ts_poet_1.Lambda(new Map([['err', ts_poet_1.TypeNames.anyType('Error')]]), ts_poet_1.TypeNames.VOID));
        }
...
function generateStreamingRpcMethod(options, typeMap, fileDesc, serviceDesc, methodDesc) {
    let requestFn = ts_poet_1.FunctionSpec.create(methodDesc.name);
    if (options.useContext) {
        requestFn = requestFn.addParameter('ctx', ts_poet_1.TypeNames.typeVariable('Context'));
    }
    ...
    return requestFn
        .addParameter('request', inputType)
        .addParameter('onMessage', new ts_poet_1.Lambda(new Map([['msg', outputType]]), ts_poet_1.TypeNames.VOID))
        .addParameter('onEnd', new ts_poet_1.Lambda(new Map([['err', ts_poet_1.TypeNames.anyType('Error')]]), ts_poet_1.TypeNames.VOID))
        .addStatement('const data = %L.encode(request).finish()', inputType)
        .addStatement('const _onMessage = data => onMessage(%L.decode(new %T(data)))', outputType, 'Reader@protobufjs/minimal')
        .addStatement('this.rpc.invoke(%L"%L.%L", %S, %L, %L, %L)', options.useContext ? 'ctx, ' : '', // sneak ctx in as the 1st parameter to our rpc call
    fileDesc.package, serviceDesc.name, methodDesc.name, 'data', '_onMessage', 'onEnd')
        .addStatement('return')
        .returns('void');
}
...
function generateRpcType(options) {
    ...
    let invokeFn = ts_poet_1.FunctionSpec.create('invoke');
    if (options.useContext) {
    ...
    invokeFn = invokeFn
        ...
        .addParameter('method', ts_poet_1.TypeNames.STRING)
        .addParameter('data', data)
        .addParameter('onMessage', new ts_poet_1.Lambda(new Map([['msg', data]]), ts_poet_1.TypeNames.VOID))
        .addParameter('onEnd', new ts_poet_1.Lambda(new Map([['err', ts_poet_1.TypeNames.anyType('Error')]]), ts_poet_1.TypeNames.VOID))
        .returns(ts_poet_1.TypeNames.VOID);
